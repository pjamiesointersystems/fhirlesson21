...
FHIR_BASE = "https://localhost:8443/csp/healthshare/demo/fhir/r4"
...
def get_patient(patient_id: str, token: str) -> Patient:
    ...
    r = requests.get(url, headers=_headers(token), verify="intersystems.crt")
    r.raise_for_status()
...
def search_patients(params: str, token: str) -> List[Patient]:
    ...
    try:
        response = requests.get(
            ...
            verify="intersystems.crt",  # Adjust path or set verify=False for local dev.
        )
